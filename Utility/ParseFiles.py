...
def ExtractVitals(lst, FILTER=None):
	...
	for nb in lst:
		...
		if lam1.shape[0] == 2:		
			...
			AB.append(A1[1])
			lamB.append(lam1[1])
			QB.append(lam1[1]/fwhm1[1])
			AR.append(A1[2])
			lamR.append(lam1[2])
			QR.append(lam1[2]/fwhm1[2])	
			lSplit.append(lam1[2]-lam1[1])			
		elif lam1.shape[0] == 1: 
			lamB.append(lam1[0])
			QB.append(lam1[0]/fwhm1[0])
			AB.append(A1[0])
...
def ExtractVitals2(lst, FILTER=None):
	'''
	Extract data from a list of uPL objects, assuming that that there are maximally two resonances in each.
	Average multiple measurements into one set of fits.
	'''
	...
	for nb in lst:
		...
		wt1 	= np.array(wt1)
		lamF.extend(lam1)
		Qtmp 	= np.array(lam1)/np.array(fwhm1)
		QF.extend(Qtmp.tolist())
		AF.extend(A1)		
		print "\tFound %d peaks for %s %s (%d,%d)" % (len(lam1), nb.GetSampleID(), nb.GetCellID(), nb.GetRow(), nb.GetColumn())
	return [np.array(AF),np.array(lamF),np.array(QF)]
...
def FitCell(sID, dirname, READ0216=False):
	...
	onlyfiles 	= filter(lambda x: len(x.split(".dat")) > 1, onlyfiles)
	nbupl 		= []
	for f in onlyfiles:
		if READ0216:
		...
		if cID == "H4" and grat == 1200 and num > 2:
			continue
		if cID == "E4" and grat == 1200:
			continue
