function [ch,tph]=Read_PTU2 % Read PicoQuant Unified TTTR Files
...
function [ch,t] = GotPhoton(TimeTag, Channel, DTime)
  global isT2;
  global fpout;
  global RecNum;
  global MeasDesc_GlobalResolution;
...
%% Read PicoHarp T2
function [marker,timeps]=ReadPT2
    global fid;
    global fpout;
    global RecNum;
    global TTResult_NumberOfRecords; % Number of TTTR Records in the File;
    ...
    marker  = zeros(1,TTResult_NumberOfRecords);
    timeps  = zeros(1,TTResult_NumberOfRecords);
    ...
    for i=1:TTResult_NumberOfRecords
        ...
        if (chan >= 0) && (chan <= 4)
            marker(i)   = chan+1;
            timeps(i)   = (timetag * MeasDesc_GlobalResolution * 1e12);
        end;
